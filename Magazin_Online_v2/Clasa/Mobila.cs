...
namespace Magazin_Online_v2
{
    public class Mobila : Product
    {
        ...
        public Mobila(string categorie, int id, string name, string description, string date, string image, double price, int stock, string numeMobila, string culoare, int lungime, int latime, int inaltime) : base(categorie, id, name, description, date, image, price, stock)
        {
        ...
        public Mobila(string[] text) : base(text[0], int.Parse(text[1]), text[2], text[3], text[4], text[5], double.Parse(text[6]), int.Parse(text[7]))
        {
